...
export const buildCalculationContext = ({
  ...
  const approxTransitDirection = approxTransitDiffMinutes === 0
    ? "exact"
    : approxTransitDiffMinutes > 0
    ? "later"
    : "earlier";
  ...
  const highLatitudeRuleValue =
    typeof parameters.highLatitudeRule === "string"
  ...
  return {
    ...
    obliquity: {
      mean: meanObliquityValue,
      apparent: apparentObliquityValue,
      nutationLongitude,
      nutationObliquity,
    },
    solar: {
    ...
    geometry: {
      asrShadow,
      latitudeDeclinationSeparation,
    },
    adjustments: {
      approxTransitDiffMinutes,
      approxTransitDirection,
    },
    prayerTimes,
    sunnahTimes,
  };
